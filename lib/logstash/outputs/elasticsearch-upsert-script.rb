...
class LogStash::Outputs::ElasticSearchUpsertScript < LogStash::Outputs::Base
  attr_reader :client
  ...
  config_name "elasticsearch-upsert-script"
  ...
  # Define the script to be executed for the update
  # in case of version conflict
  config :script, :validate => :string
  
  # Define the script parameters
  # The script parameters will be passed at source in case of an updatescript action.
  config :script_params, :validate => :hash, :default => nil

  # Must the script run in any case
  config :_scripted_upsert, :validate => :boolean
  
  # Define the script language
  config :script_lang, :validate => :string, :default => "groovy"  
...
#      LogStash::Environment.load_elasticsearch_jars!
  ...
  def receive(event)
	...
	if @action == 'update'
	  params[:_script] = @script if @script
	  params[:_script_params]= @script_params if @script_params
	  params[:_scripted_upsert]= @scripted_upsert if @scripted_upsert
	  params[:_script_lang]= @script_lang if @script_lang
    end
